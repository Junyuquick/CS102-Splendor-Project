 ...
 *
 * This base class builds the common layout, tracks UI selection state, and provides helper
 * methods for constructing pending moves from the current interaction mode.
 * ideated with ChatGPT-5.3, to efficiently build the skeleton of our code before we did detailed modifications, such as position, color, logical conditions, etc
 */
abstract class AbstractSwingSplendorFrame extends JFrame {
     ...
     * @param state current game state, or null before a multiplayer game starts
    ...
    /**
     * Applies frame-level configuration such as sizing, layout, and base panel geometry.
     */
    ...
    /**
     * Creates the top status bar used across game windows.
     *
     * @param showLobbyCenter whether to render the lobby status label in the center slot
     * @param showStartButton whether to include the start-game button on the right side
     * @return configured top-bar panel
     */
    // Partially generated by ChatGPT-5.3. used to generate the standard structure of code for the design without values included.
    ...
    /**
     * Creates the left-side bank section with token counts and token-selection buttons.
     *
     * @return configured bank panel
     */
    // Partially generated by ChatGPT-5.3. used to generate the standard structure, personally fixed margin issues with setMargin()
    ...
    /**
     * Creates the center gameplay section containing market cards and nobles.
     *
     * @return configured market container
     */
    // Partially generated by ChatGPT-5.3. used to generate the standard structure of code for the design without values included.
    ...
    /**
     * Creates the right-side section with player summaries and reserved-card list.
     *
     * @param playerRows preferred row count for player summary slots
     * @return configured players panel
     */
    // Partially generated by ChatGPT-5.3. used to generate the standard structure of code for the design without values included.
    ...
    /**
     * Creates the bottom section containing action buttons, mode help, and event log.
     *
    ...
    /**
     * Mounts the center and bottom sections into a split pane in the frame body.
     *
     * @param center center gameplay panel
     ...
     * note: reason i didnt make it abstract is cuz its optional - jy
     *
    ...
    /**
     * Sets the current interaction mode and clears transient selection state.
     *
     ...
     *
     * @return pending move, or null if the current selection is incomplete
     */
    ...
    /**
     * Clears market controls when no active game state is available.
     */
    ...
    /**
     * Rebuilds all market slots from the board's face-up cards and empty positions.
     */
    ...
    /**
     * Creates a baseline-styled button used for any market slot.
     *
     * @return unbound market button shell
     */
    ...
    /**
     * Configures visuals, tooltip, and click behavior for an occupied market slot.
     *
     * @param button slot button to configure
     * @param key board key for tooltip display, for example a1
     * @param card face-up card currently occupying the slot
     */
    // Partially generated by ChatGPT-5.3. gave ai an idea and had it generate template for efficiency in coding
    ...
    /**
     * Configures an unoccupied market slot as disabled placeholder content.
     *
     * @param button slot button to configure
     * @param key board key for the empty slot
     */
    ...
    /**
     * Creates the placeholder card shown when no nobles are available.
     *
     * @return noble placeholder panel
     */
    ...
    /**
     * Builds one noble tile panel with visual content and tooltip details.
     *
     * @param noble noble tile to render
     * @param index zero-based display index used in labels/tooltips
     * @return configured noble tile panel
     */
    ...
    /**
     * Creates the label content for a noble tile, preferring artwork when available.
     *
     * @param noble noble tile to render
     * @param index zero-based display index for fallback text
     * @return configured noble label
     */
    ...
    /**
     * Resets all market card buttons to disabled state with default border styling.
     */
    ...
    /**
     * Converts market coordinates to the stable slot key used in {@link #cardButtons}.
     *
     * @param tier board tier (1-indexed)
     * @param col column index (0-indexed)
     * @return key formatted as column letter plus tier, for example a1
     */
    ...
    /**
     * Computes legality for the given card under the current mode and updates button state.
     *
     * @param current active player
     * @param card card represented by the button
    ...
    /**
     * Applies selection and legality border styling to a market card button.
     *
     * @param button card button to style
     * @param card card represented by the button
     * @param legal whether the corresponding move is currently legal
     */
    ...
    /**
     * Determines whether a token button should be enabled for the current mode.
     *
     * @param current active player
     * @param allowInteractions whether user interaction is globally allowed
     * @param color token color represented by the button
     * @param totalSelected total number of currently selected token units
     * @return true if the button should be clickable
     */
    ...
    /**
     * Checks whether a color is selectable in TAKE_THREE mode.
     *
     * @param color color to test
     * @param totalSelected current number of selected token units
     * @return true when selecting this color would keep the selection valid
     */
    ...
    /**
     * Checks whether taking same-color tokens for the given color is legal.
     *
     * @param current active player
     * @param color candidate color for TAKE_TWO mode
     * @return true if a preview TAKE_TWO move validates
     */
     ...
     *
     * @return true to include bank counts in button labels
     */
     ...
     *
     * @param player player to check
     * @return true if a legal reserve move exists
     */
     ...
     *
     * @param player player to check
     * @return true if a legal purchase exists
     */
     ...
     *
     * @param player player to check
     * @return true if a legal return move exists
     */
    ...
    /**
     * Recursively searches combinations of distinct colors for at least one legal TAKE_THREE move.
     *
     * @param player player to test move legality for
     * @param colors ordered list of candidate colors
     * @param startIndex recursion cursor into colors
     * @param remaining number of additional colors still required in the combination
     * @param selected mutable map containing the current partial selection
     * @return true once a legal combination is found
     */
     ...
     *
     * @param key board key such as a1
