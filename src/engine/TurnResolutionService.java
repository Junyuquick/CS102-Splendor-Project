...
public final class TurnResolutionService {
    ...
    public TurnResolutionResult resolveTurn(GameState state, Player player, Move move) {
        ...
        Map<GemColor, Integer> discardedTokens = turnPostProcessor.enforceTokenLimit(state, player);
        List<NobleTile> assignedNobles = turnPostProcessor.assignBestAvailableNobles(state, player);
        TurnAdvanceResult turnAdvanceResult = turnProgressionService.progressTurn(state, winnerChecker, turnManager);
